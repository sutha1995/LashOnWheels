...
Never commit `.env` or service-role keys. Apply the freelancer profile, service catalog, availability, booking, payment-status, and notification migrations before using these flows with real accounts. Payment checkout, portfolio storage, and location features will be added in later PRD phases.
