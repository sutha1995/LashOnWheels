create table if not exists public.booking_treatment_consents (
  booking_id uuid primary key references public.bookings(id) on delete cascade,
  customer_id uuid not null references public.profiles(id) on delete cascade,
  service_consent boolean not null check (service_consent),
  health_disclosure_consent boolean not null default false,
  terms_version text not null default '2026-10-12',
  consented_at timestamptz not null default now()
);

alter table public.booking_treatment_consents enable row level security;
create policy "Customers can view their own treatment consent" on public.booking_treatment_consents for select using (auth.uid() = customer_id);
create policy "Assigned freelancers can view treatment consent" on public.booking_treatment_consents for select using (exists (select 1 from public.bookings where bookings.id = booking_id and bookings.freelancer_id = auth.uid()));

drop function if exists public.create_booking(uuid, date, time, time, text, text, double precision, double precision, text, text, boolean);
create function public.create_booking(
  p_freelancer_service_id uuid, p_scheduled_date date, p_start_time time, p_end_time time,
  p_customer_note text default '', p_service_address text default '', p_latitude double precision default null, p_longitude double precision default null,
  p_allergies text default '', p_medications text default '', p_health_disclosure_consent boolean default false, p_service_consent boolean default false
)
...
begin
  ...
  if not p_service_consent then raise exception 'Treatment consent is required before requesting a booking.'; end if;
  ...
  insert into public.booking_treatment_consents (booking_id, customer_id, service_consent, health_disclosure_consent) values (booking_row.id, auth.uid(), true, health_details_present and p_health_disclosure_consent);
...
revoke all on function public.create_booking(uuid, date, time, time, text, text, double precision, double precision, text, text, boolean, boolean) from public;
grant execute on function public.create_booking(uuid, date, time, time, text, text, double precision, double precision, text, text, boolean, boolean) to authenticated;
